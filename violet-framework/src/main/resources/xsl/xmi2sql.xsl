...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="seq">
		...
		<xsl:if test="contains($node//UML:DataType/@xmi.idref, '++')">
			...
			<!-- <xsl:text> SELECT setval('</xsl:text> <xsl:value-of select="$table" 
				/> <xsl:text>_</xsl:text> <xsl:value-of select="@name" /> <xsl:text>', 1);&#xa;</xsl:text> 
				<xsl:text> use sequence by nextval('</xsl:text> <xsl:value-of select="$table" 
	...
	<xsl:template match="UML:Association">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:if
					test="count(//UML:Class[@xmi.id=$idFils]//UML:Attribute[@visibility='private']) &gt; 0 and count(//UML:Class[@xmi.id=$idPere]//UML:Attribute[@visibility='private']) &gt; 0 ">
					<xsl:text>&#xa;</xsl:text>
					<xsl:text>create table </xsl:text>
					<xsl:value-of select="$name" />
					<xsl:text>(&#xa;</xsl:text>
	...
	<xsl:template match="UML:Generalization">
		...
		<xsl:if
			...
			<xsl:if
				test="contains(//UML:Class[@xmi.id=$idParent]//UML:Attribute/@visibility, 'private')">
				<xsl:variable name="pkvalue">
					<xsl:for-each

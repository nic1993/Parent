<xsl:stylesheet version="1.0"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:UML='org.omg.xmi.namespace.UML'
			...
			<newFile path="mpd.class.violet" name="mpd.class.violet">
				<java version="1.5.0" class="java.beans.XMLDecoder">
					<object class="com.horstmann.violet.ClassDiagramGraph">
						...
						<xsl:apply-templates
							select="//UML:Namespace.ownedElement/UML:Generalization" />
						...
						<string>
							...
							<xsl:for-each select=".//UML:Attribute[@visibility='private']">
								...
								<xsl:if test="@visibility='private'">
									+
								</xsl:if><!-- modification for mpd2sql -->
	...
	<xsl:template match="UML:Association">
		<xsl:variable name="startId">
			<xsl:value-of select=".//UML:AssociationEnd[1]//UML:Class/@xmi.idref" />
		</xsl:variable>
		<xsl:variable name="startLower">
			<xsl:value-of
				select=".//UML:AssociationEnd[1]//UML:MultiplicityRange[@xmi.id]/@lower" />
		</xsl:variable>
		<xsl:variable name="startUpper">
			<xsl:value-of
				select=".//UML:AssociationEnd[1]//UML:MultiplicityRange[@xmi.id]/@upper" />
		</xsl:variable>
		<xsl:variable name="endId">
			<xsl:value-of select=".//UML:AssociationEnd[2]//UML:Class/@xmi.idref" />
		</xsl:variable>
		<xsl:variable name="endLower">
			<xsl:value-of
				select=".//UML:AssociationEnd[2]//UML:MultiplicityRange[@xmi.id]/@lower" />
		</xsl:variable>
		<xsl:variable name="endUpper">
			<xsl:value-of
				select=".//UML:AssociationEnd[2]//UML:MultiplicityRange[@xmi.id]/@upper" />
		</xsl:variable>

		<xsl:variable name="test1"
			select="($startUpper = 1 and $endUpper &lt; 1) or ($startUpper &lt; 1 and $endUpper = 1)" />
		...
		<xsl:if test="$test1 or $test2">
			<void method="connect">
				<object class="com.horstmann.violet.ClassRelationshipEdge">
					...
					<xsl:if
						test=".//UML:AssociationEnd[1]/@isNavigable='true' and .//UML:AssociationEnd[2]/@isNavigable='false' ">
						<void property="startArrowHead">
							<object class="com.horstmann.violet.ArrowHead">
					...
					<xsl:if
						test=".//UML:AssociationEnd[2]/@isNavigable='true' and .//UML:AssociationEnd[1]/@isNavigable='false' ">
						<void property="endArrowHead">
							<object class="com.horstmann.violet.ArrowHead">
								<xsl:attribute name="field">V</xsl:attribute>
							</object>
				...
				<xsl:for-each select=".//UML:Class">
					<object>
						<xsl:attribute name="idref"><xsl:value-of
							select="@xmi.idref" /></xsl:attribute>
					</object>
				</xsl:for-each>
			</void>
		</xsl:if>
	...
	<xsl:template match="UML:Generalization">
		<void method="connect">
			...
			<object>
				<xsl:attribute name="idref"><xsl:value-of
					select=".//UML:Generalization.child/UML:Class/@xmi.idref" /></xsl:attribute>
			</object>
			<object>
				<xsl:attribute name="idref"><xsl:value-of
					select=".//UML:Generalization.parent/UML:Class/@xmi.idref" /></xsl:attribute>
			</object>
		</void>
	</xsl:template>
	...
	<xsl:template match="UML:Abstraction">
		<void method="connect">
			...
			<object>
				<xsl:attribute name="idref"><xsl:value-of
					select=".//UML:Dependency.client/UML:Class/@xmi.idref" /></xsl:attribute>
			</object>
			<object>
				<xsl:attribute name="idref"><xsl:value-of
					select=".//UML:Dependency.supplier/UML:Class/@xmi.idref" /></xsl:attribute>
			</object>
		</void>
	</xsl:template>
	...
	<xsl:template name="GetAssociation">
		...
		<xsl:for-each
			select="//UML:Association[//UML:AssociationEnd//UML:Class/@xmi.idref = $node]">
			<xsl:variable name="startId">
				<xsl:value-of select=".//UML:AssociationEnd[1]//UML:Class/@xmi.idref" />
			</xsl:variable>
			<xsl:variable name="startLower">
			...
			<xsl:choose>
				<xsl:when
					test="($startUpper = 1 and $endUpper &lt; 1) or ($startUpper &lt; 1 and $endUpper = 1)">
					<!-- relation 1 vers N -->
					<xsl:variable name="idPere">
						<xsl:if test="$endUpper &lt; 1">
							<xsl:value-of select="$endId" />
						</xsl:if>
						<xsl:if test="$startUpper &lt; 1">
					...
					<xsl:if test="$idPere = $node">
						<xsl:if
							test="count(//UML:Class[@xmi.id=$idFils]//UML:Attribute[@visibility='private']) &gt; 0">
							<xsl:if test="$type = 'attribute'">
								<xsl:text>+ </xsl:text>
								<xsl:value-of
									select="//UML:Class[@xmi.id=$idFils]//UML:Attribute[@visibility='private']/@name" />
								<xsl:text>_</xsl:text>
								<xsl:value-of select="//UML:Class[@xmi.id=$idFils]/@name" />
								<xsl:text> : </xsl:text>
								<xsl:apply-templates
									select="//UML:Class[@xmi.id=$idFils]//UML:Attribute[@visibility='private']//UML:DataType" />
								<xsl:text>&#xa;</xsl:text>
							</xsl:if>
				...
				<xsl:when
					test="($startUpper = 1 and $endUpper = 1)  and ($startLower = $endLower)">
					<!-- relation 1 vers 1 -->
					<xsl:variable name="idPere">
						<xsl:choose>
							<xsl:when
								test="(.//UML:AssociationEnd[2]/@aggregation='composite') or (.//UML:AssociationEnd[2]/@aggregation='aggregate')">
								<xsl:value-of select="$endId" />
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$startId" />
					...
					<void method="addNode">
						<object>
							<xsl:attribute name="id"><xsl:value-of
								select="$idPere" />-1</xsl:attribute>
							<xsl:attribute name="class">com.horstmann.violet.ClassNode</xsl:attribute>
							<void property="attributes">
								<void property="text">
									<string>
										<xsl:for-each
											select="//UML:Class[@xmi.id=$idFils]//UML:Attribute[@visibility='private']">
											<xsl:text>+ </xsl:text>
											<xsl:value-of select="@name" />
											<xsl:text>_</xsl:text>
											<xsl:value-of select="//UML:Class[@xmi.id=$idFils]/@name" />
										...
										<xsl:for-each
											select="//UML:Class[@xmi.id=$idPere]//UML:Attribute[@visibility='private']">
											<xsl:text>+ </xsl:text>
											<xsl:value-of select="@name" />
											<xsl:text>_</xsl:text>
											<xsl:value-of select="//UML:Class[@xmi.id=$idPere]/@name" />
							...
							<void property="methods">
								<void property="text">
									<string>
										...
										<xsl:text> , </xsl:text>
										<xsl:value-of
											select="//UML:Class[@xmi.id=$idPere]//UML:Attribute[@visibility='private']/@name" />
										<xsl:text> : </xsl:text>
										<xsl:apply-templates
											select="//UML:Class[@xmi.id=$idPere]//UML:Attribute[@visibility='private']//UML:DataType" />
										<xsl:text> , </xsl:text>
										<xsl:value-of
											select="//UML:Class[@xmi.id=$idPere]//UML:Attribute[@visibility='private']/@name" />
										<xsl:text>_</xsl:text>
						...
						<object class="java.awt.geom.Point2D$Double">
							<void method="setLocation">
								<double>
									<xsl:value-of
										select="(//UML:Class[@xmi.id=$idPere]//violet:void[@method='setLocation']/violet:double[1] + //UML:Class[@xmi.id=$idFils]//violet:void[@method='setLocation']/violet:double[1]) div 2" />
								</double>
								<double>
									<xsl:value-of
										select="(//UML:Class[@xmi.id=$idPere]//violet:void[@method='setLocation']/violet:double[2] + //UML:Class[@xmi.id=$idFils]//violet:void[@method='setLocation']/violet:double[2]) div 2" />
								</double>
							</void>
						</object>
					</void>
	...
	<xsl:template name="GetGeneralization">
		...
		<xsl:for-each
			select="//UML:Generalization[//UML:Generalization.child//UML:Class/@xmi.idref = $node]">
			...
			<xsl:if test="$type = 'method'">
				<xsl:if
					test="count(//UML:Class[@xmi.id=$idParent]//UML:Attribute[@visibility= 'private']) &gt; 0">
					<xsl:text>getPK(</xsl:text>
					<xsl:for-each
						select="//UML:Class[@xmi.id=$idParent]//UML:Attribute[@visibility='private']">
						<xsl:value-of select="@name" />
						<xsl:text> : </xsl:text>
						<xsl:apply-templates select=".//UML:DataType" />
						<xsl:if test="string-length(.//UML:Expression/@body) &gt; 0">

...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="UML:Class">
		...
		<newFile>
			...
			<xsl:variable name="compter">
				<xsl:value-of
					select="count(.//UML:GeneralizableElement.generalization//UML:Generalization)+count(.//UML:ModelElement.clientDependency//UML:Abstraction)" />
			</xsl:variable>
			...
			<xsl:if
				test="count(.//UML:ModelElement.clientDependency//UML:Abstraction) &gt; 0">
				<xsl:text>,</xsl:text>
			</xsl:if>
	...
	<xsl:template name="getImport">
		<xsl:param name="node" />
		<xsl:for-each
			select="$node//UML:GeneralizableElement.generalization//UML:Generalization">
		...
		<xsl:for-each
			select="$node//UML:ModelElement.clientDependency//UML:Abstraction">
			<xsl:text>from </xsl:text>
			<xsl:variable name="name">
				<xsl:apply-templates select="." />
			</xsl:variable>
	...
	<xsl:template name="getClassType">
		...
		<xsl:if test='count($node//UML:Stereotype) &gt; 0'>
			...
			<xsl:value-of
				select="//UML:Stereotype[@xmi.id=$id]/UML:Stereotype.baseClass" />
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="UML:Generalization">
		...
		<xsl:variable name="idrefParent">
			<xsl:value-of
				select="//UML:Generalization[@xmi.id=$idref]/UML:Generalization.parent/UML:Class/@xmi.idref" />
		</xsl:variable>
	...
	<xsl:template match="UML:Abstraction">
		...
		<xsl:variable name="idrefParent">
			<xsl:value-of
				select="//UML:Abstraction[@xmi.id=$idref]/UML:Dependency.supplier/UML:Class/@xmi.idref" />
		</xsl:variable>
		<xsl:value-of select="//UML:Class[@xmi.id=$idrefParent]/@name" />
	</xsl:template>
	...
	<xsl:template match="UML:DataType">
		...
		<!-- <xsl:variable name="idref"> <xsl:value-of select="@xmi.idref" /> </xsl:variable> 
	...
	<xsl:template name="Assoc">
		...
		<xsl:param name="id" />
		<xsl:for-each
			select="$node//UML:AssociationEnd[.//UML:Class/@xmi.idref != $id]">
			<xsl:apply-templates select="." />
		</xsl:for-each>
	</xsl:template>

...
public class StepOneCenterRightPanel extends JPanel{
    ...
    private void init()
    {
    	...
    	this.add(bottomPanel,new GBC(0, 2, 1, 1).setFill(GBC.BOTH).setWeight(1, 1));
    	buttonListen();
    }
    	...
    	sidbarbButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(mainFrame.getCenterTabPanel().getComponent(0).equals(mainFrame.getStepOneCenterSequenceTabbedPane())
					&& (mainFrame.getSequenceWorkspaceList().size() != 0 || mainFrame.getUseCaseWorkspaceList().size() != 0))
				{
					if(SideBarFlag == 0){
				    int loaction = mainFrame.getStepOneCenterSequenceTabbedPane().getSelectedIndex();
					mainFrame.getSequenceWorkspaceList().get(loaction)
					.getAWTComponent().getScrollableSideBar().setVisible(false);
					mainFrame.getSequenceWorkspaceList().get(loaction)
					.getAWTComponent().validate();
					mainFrame.getSequenceWorkspaceList().get(loaction)
					.getAWTComponent().revalidate();
					SideBarFlag = 1;
					}
					else if(SideBarFlag == 1){
						int loaction = mainFrame.getStepOneCenterSequenceTabbedPane().getSelectedIndex();
						mainFrame.getSequenceWorkspaceList().get(loaction)
						.getAWTComponent().getScrollableSideBar().setVisible(true);
						mainFrame.getStepOneCenterSequenceTabbedPane().getSequenceDiagramTabbedPane().get(loaction).repaint();
						mainFrame.getSequenceWorkspaceList().get(loaction)
						.getAWTComponent().revalidate();
						SideBarFlag = 0;
						}

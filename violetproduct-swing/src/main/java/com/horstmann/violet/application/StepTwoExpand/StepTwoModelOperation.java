...
public class StepTwoModelOperation extends JPanel{
	...
	public void init()
	{
		...
		layout.setConstraints(otherPanel, new GBC(0, 1, 5, 1).setFill(GBC.BOTH).setWeight(1, 1).setInsets(0, 28, 0, 0));
	}
...
//		rightpanel.add(label,new GBC(0, 0,1,1).setFill(GBC.BOTH).setWeight(0, 1).setInsets(10, 40, 5, 0));
			...
			public void actionPerformed(ActionEvent e) {
				...
				{ 
				   //添加弹出框
					...
					for(j = 0;j < number;++j){
						int i = 0; //标记位置
						StepTwoModelExpandPanel stepTwoModelExpandPanel = new StepTwoModelExpandPanel();
						stepTwoModelExpandPanel.getTitleLabel().setText("用户"+(j+1));
						stepTwoModelExpandPanel.getTabelPanel().setLayout(new GridBagLayout());
						for(String key : ucMap.keySet())
						{
							...
							if(interfaceUCRelations.size() > 1 && !key.equals("Use Case3"))
							{
								...
								stepTwoMatrixPanel.getTitleLabel().setText(key);
								stepTwoMatrixPanel.getTabelPanel().add(scenceTabelPanel);
								stepTwoModelExpandPanel.getTabelPanel().add(stepTwoMatrixPanel, new GBC(0,i).setFill(GBC.BOTH).setWeight(1, 0));
								i++;
								relations.clear();
							}
						}
						stepTwoModelExpandTabbedPane.getmodelExpandPanel().add(stepTwoModelExpandPanel, new GBC(0, j).setFill(GBC.BOTH).setWeight(1, 0));
					}
					...
					{    
						for(InterfaceUCRelation interfaceUCRelation:interfaceUCRelations)
						{
					   	 ...
					   	 for(int i = 0;i <number;i++)
					   	 {
					   		 ...
					   		 if(location != -1)
					   		 {
					   			JPanel panel = ((StepTwoModelExpandPanel) stepTwoModelExpandTabbedPane.getmodelExpandPanel().getComponent(i)).getTabelPanel();
	...
	{
		JPanel panel = ((StepTwoModelExpandPanel) stepTwoModelExpandTabbedPane.getmodelExpandPanel().getComponent(0)).getTabelPanel();
